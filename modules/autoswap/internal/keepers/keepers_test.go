...
async function initializeTokenWithETHAsStock(boss) {
    ...
    result = await factory.createPair(usd.address, eth, impl.address);
    pairAddr = result.logs[0].args.pair;
    console.log("pairAddr: ", pairAddr)
    pair = await pairCon.at(pairAddr)
    stock = await pair.stock.call()
...
contract("deal with pool", async (accounts) => {
    ...
    it("insert buy order which can not be dealt", async ()=>{
        ...
        console.log(amounts[0],amounts[1])
        amounts = syncAmountFromEventSync(result.logs[2].args.reserveStockAndMoney)
        console.log(amounts[0],amounts[1])
        reserves = await pair.getReserves.call();
        console.log("reserve stock amount: ", reserves.reserveStock.toNumber())
        console.log("reserve money amount: ", reserves.reserveMoney.toNumber())
...
contract("deal after donate and sync", async (accounts) => {
    ...
    it("deal with pool", async () => {
        ...
        let balance = await btc.balanceOf.call(taker);
        assert.equal(balance.toNumber(), 906);
    });
});
...
contract("pair with weth token", async (accounts) => {
    ...
    it("addMarketOrder to buy eth", async ()=>{
       ...
       let reserves = await pair.getReserves.call();
       assert.equal(reserves.reserveStock.toNumber(),9992,"reserve stock amount is not correct")
    ...
    it("addMarketOrder to sell eth", async ()=>{
       ...
       let reserves = await pair.getReserves.call();
       assert.equal(reserves.reserveStock.toNumber(),10091,"reserve stock amount is not correct")
       assert.equal(reserves.reserveMoney.toNumber(),1000151,"reserve money amount is not correct")
       console.log("reserve stock amount: ", reserves.reserveStock.toNumber())
       console.log("reserve money amount: ", reserves.reserveMoney.toNumber())
...
contract("pair with eth token as stock", async (accounts) => {
    ...
    it("initialize pair with eth/usd", async () => {
        await initializeTokenWithETHAsStock(boss);
    });
});
...
contract("pair with eth token", async (accounts) => {
    ...
    it("initialize pair with usd/eth", async () => {
    ...
    it("addMarketOrder to buy eth", async ()=>{
        ...
        let reserves = await pair.getReserves.call();
        assert.equal(reserves.reserveStock.toNumber(),9902,"reserve stock amount is not correct")
        assert.equal(reserves.reserveMoney.toNumber(),101_0030,"reserve money amount is not correct")
        console.log("reserve stock amount: ", reserves.reserveStock.toNumber())
        console.log("reserve money amount: ", reserves.reserveMoney.toNumber())
    ...
    it("addMarketOrder to sell eth", async ()=>{
        ...
        let reserves = await pair.getReserves.call();
        assert.equal(reserves.reserveStock.toNumber(),10001,"reserve stock amount is not correct")
        assert.equal(reserves.reserveMoney.toNumber(),1000062,"reserve money amount is not correct")
        console.log("reserve stock amount: ", reserves.reserveStock.toNumber())
        console.log("reserve money amount: ", reserves.reserveMoney.toNumber())
